# **RustPlusBot** plugin example: vendinglogany

This plugin example demonstrates how to log only vending machine item changes in a specific location on the server to the Notifications Discord Channel (or the Plugin Notification channel if set in the bot's config).
